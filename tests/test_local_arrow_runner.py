...
def test_from_list_special_values(runner):
    ...
    data = {
        'strings': ['normal', '', 'test'],
        'numbers': [1, 0, -5],
        'floats': [1.5, 0.0, -3.14]  # Avoid float('inf') which causes serialization issues
    }
    op = FromListOp(data=data)
    result = runner.materialize(op).to_arrow()
    ...
    assert result['floats'].to_pylist() == [1.5, 0.0, -3.14]


# SelectColumnOp Tests
def test_select_column_basic(multi_column_dataset, runner):
    """Test basic SelectColumnOp functionality."""
...
def test_select_text_column(multi_column_dataset, runner):
    """Test SelectTextColumnOp functionality."""
...
def test_select_vector_column(runner):
    """Test SelectVectorColumnOp functionality."""
...
def test_select_categorical_column(multi_column_dataset, runner):
    """Test SelectCategoricalColumnOp functionality."""
...
def test_select_train_test_split_column(runner):
    """Test SelectTrainTestSplitColumnOp functionality (will fail if not implemented)."""
    data = {
...
def test_assign_train_test_split_op(runner):
    ...
    from krnel.graph.dataset_ops import AssignTrainTestSplitOp, SelectTextColumnOp
    
    # Create a dataset with text data to hash for train/test split
    data = {'text': ['sample1', 'sample2', 'sample3', 'sample4']}
    dataset = FromListOp(data=data)
    text_col = SelectTextColumnOp(column_name='text', dataset=dataset)
    
    op = AssignTrainTestSplitOp(
        hash_column=text_col,
        test_size=0.5,
        random_state=42
    )
...
def test_jinja_templatize_op(runner):
    """Test JinjaTemplatizeOp functionality (will fail if not implemented)."""
